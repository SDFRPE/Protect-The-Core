...
public class MochilaCommand implements CommandExecutor {
    ...
    public MochilaCommand(PTC plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("ptc.echest")) {
            player.sendMessage(c("&cNecesitas el permiso &6ptc.echest &cpara usar este comando."));
            return true;
        }

        if (plugin.getGameManager().getGlobalSettings().isLobbyMode()) {
            player.sendMessage(c("&c✖ La mochila no está disponible en el servidor de lobby."));
            player.sendMessage(c("&7Únete a una partida para usar tu mochila."));
            return true;
        }

        if (Statics.gameStatus != GameStatus.IN_GAME) {
            player.sendMessage(c("&c✖ La mochila solo está disponible durante una partida."));
